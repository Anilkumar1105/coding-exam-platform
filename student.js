...
export async function listActiveExams() {
  ...
  const snap = await getDocsCached(q, "active-exams", 20000);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

export async function getExamById(examId) {
  const snap = await getDocCached(doc(db, "exams", examId), `exam:${examId}`, 20000);
...
export async function listSchedulesForExam(examId) {
  ...
  const snap = await getDocsCached(q, `exam-schedules:${examId}`, 20000);
  return snap.docs
...
export async function getQuestionsForExam(examId) {
  ...
  const snap = await getDocsCached(q, `exam-questions:${examId}`, 120000);
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
